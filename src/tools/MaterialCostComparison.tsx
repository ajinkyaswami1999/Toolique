...
export default function MaterialCostComparison() {
  // Input States
  const [weight, setWeight] = useState<number>(100);
  const [plaPrice, setPlaPrice] = useState<number>(1500);
  const [petgPrice, setPetgPrice] = useState<number>(1700);
  const [absPrice, setAbsPrice] = useState<number>(1600);
  const [nylonPrice, setNylonPrice] = useState<number>(3500);
  ...
  useEffect(() => {
    try {
      ...
      const pla = (weight / 1000) * plaPrice;
      const petg = (weight / 1000) * petgPrice;
      const abs = (weight / 1000) * absPrice;
      const nylon = (weight / 1000) * nylonPrice;
      return {
        'PLA cost': '₹' + pla.toFixed(2),
        'PETG cost': '₹' + petg.toFixed(2),
        'ABS cost': '₹' + abs.toFixed(2),
        'Nylon cost': '₹' + nylon.toFixed(2)
      };
  ...
  }, [weight, plaPrice, petgPrice, absPrice, nylonPrice]);
        ...
        <div className="md:col-span-6 p-6 saas-card space-y-4">
          ...
          <div className="space-y-4">
            <div className="space-y-1.5">
              <label className="text-xs font-bold text-zinc-550 dark:text-zinc-400">Print Weight (g)</label>
              <input
                type="number"
                value={weight}
            ...
            <div className="space-y-1.5">
              <label className="text-xs font-bold text-zinc-550 dark:text-zinc-400">PLA Price (₹/kg)</label>
              <input
                type="number"
                value={plaPrice}
                onChange={(e) => setPlaPrice(Number(e.target.value))}
                className="saas-input"
              />
            </div>
            <div className="space-y-1.5">
              <label className="text-xs font-bold text-zinc-550 dark:text-zinc-400">PETG Price (₹/kg)</label>
              <input
                type="number"
                value={petgPrice}
                onChange={(e) => setPetgPrice(Number(e.target.value))}
                className="saas-input"
              />
            </div>
            <div className="space-y-1.5">
              <label className="text-xs font-bold text-zinc-550 dark:text-zinc-400">ABS Price (₹/kg)</label>
              <input
                type="number"
                value={absPrice}
                onChange={(e) => setAbsPrice(Number(e.target.value))}
                className="saas-input"
              />
            </div>
            <div className="space-y-1.5">
              <label className="text-xs font-bold text-zinc-550 dark:text-zinc-400">Nylon Price (₹/kg)</label>
              <input
                type="number"
                value={nylonPrice}
                onChange={(e) => setNylonPrice(Number(e.target.value))}
                className="saas-input"
              />
            </div>
          </div>
