...
export default function VolumetricFlowCalculator() {
  // Input States
  const [maxFlow, setMaxFlow] = useState<number>(15);
  const [layerHeight, setLayerHeight] = useState<number>(0.2);
  const [lineWidth, setLineWidth] = useState<number>(0.45);
  ...
  useEffect(() => {
    try {
      ...
      const maxSpeed = maxFlow / (layerHeight * lineWidth);
      return {
        'Max Print Speed Allowed': maxSpeed.toFixed(0) + ' mm/s',
        'Optimal Speed (80% Safety)': (maxSpeed * 0.8).toFixed(0) + ' mm/s'
      };
        ...
        <div className="md:col-span-6 p-6 saas-card space-y-4">
          ...
          <div className="space-y-4">
            <div className="space-y-1.5">
              <label className="text-xs font-bold text-zinc-550 dark:text-zinc-400">Max Flow Rate (mm³/s)</label>
              <input
                type="number"
                value={maxFlow}
            ...
            <div className="space-y-1.5">
              <label className="text-xs font-bold text-zinc-550 dark:text-zinc-400">Line Width (mm)</label>
              <input
                type="number"
                value={lineWidth}
