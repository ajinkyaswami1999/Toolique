...
export default function JsMinifier() {
  const [js, setJs] = useState(`function greetUser(name) {\n  // standard welcome message\n  const message = "Hello, " + name + "!";\n  console.log(message);\n  return message;\n}`);
  const [copied, setCopied] = useState(false);

  const minifyJS = (input: string) => {
    if (!input) return '';
    let minified = input;
    ...
    // Strip single line comments (cautiously, making sure not to break URLs inside quotes)
    minified = minified.replace(/(^|[^/])\/\/[^#].*/g, '$1');

    // Remove redundant whitespaces around common JS tokens
    minified = minified.replace(/\s*([=+\-*/{}()[\];:,])\s*/g, '$1');

    // Collapse multiple space sequences
    minified = minified.replace(/\s+/g, ' ');

    return minified.trim();
  };

  const output = minifyJS(js);
