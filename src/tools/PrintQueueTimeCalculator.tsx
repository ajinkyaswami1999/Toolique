...
export default function PrintQueueTimeCalculator() {
  // Input States
  const [pendingHours, setPendingHours] = useState<number>(72);
  const [activePrinters, setActivePrinters] = useState<number>(3);
  ...
  useEffect(() => {
    try {
      ...
      const queueTime = pendingHours / activePrinters;
      const days = Math.floor(queueTime / 24);
      const hoursRemaining = Math.ceil(queueTime % 24);
      return {
        'Average Dispatch Lead Time': queueTime.toFixed(1) + ' hours',
        'Lead Time Breakdown': days + ' days, ' + hoursRemaining + ' hours'
      };
        ...
        <div className="md:col-span-6 p-6 saas-card space-y-4">
          ...
          <div className="space-y-4">
            <div className="space-y-1.5">
              <label className="text-xs font-bold text-zinc-550 dark:text-zinc-400">Total Backlog Job Hours</label>
              <input
                type="number"
                value={pendingHours}
            ...
            <div className="space-y-1.5">
              <label className="text-xs font-bold text-zinc-550 dark:text-zinc-400">Available Printers in Farm</label>
              <input
                type="number"
                value={activePrinters}
                onChange={(e) => setActivePrinters(Number(e.target.value))}
                className="saas-input"
              />
            </div>
          </div>
