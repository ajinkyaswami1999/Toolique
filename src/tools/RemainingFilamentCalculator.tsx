...
export default function RemainingFilamentCalculator() {
  // Input States
  const [grossWeight, setGrossWeight] = useState<number>(650);
  const [tareWeight, setTareWeight] = useState<number>(250);
  ...
  useEffect(() => {
    try {
      ...
      const remaining = Math.max(0, grossWeight - tareWeight);
      return {
        'Net Filament Remaining': remaining.toFixed(0) + ' g',
        'Spool Capacity Left': ((remaining / 1000) * 100).toFixed(1) + '%'
      };
        ...
        <div className="md:col-span-6 p-6 saas-card space-y-4">
          ...
          <div className="space-y-4">
            <div className="space-y-1.5">
              <label className="text-xs font-bold text-zinc-550 dark:text-zinc-400">Spool + Filament Weight (grams)</label>
              <input
                type="number"
                value={grossWeight}
            ...
            <div className="space-y-1.5">
              <label className="text-xs font-bold text-zinc-550 dark:text-zinc-400">Empty Spool Weight (grams)</label>
              <input
                type="number"
                value={tareWeight}
