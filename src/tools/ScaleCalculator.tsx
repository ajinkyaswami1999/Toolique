...
export default function ScaleCalculator() {
  // Input States
  const [currentDim, setCurrentDim] = useState<number>(120);
  const [targetScale, setTargetScale] = useState<number>(150);
  ...
  useEffect(() => {
    try {
      ...
      const result = currentDim * (targetScale / 100);
      return {
        'Scaled Dimension': result.toFixed(1) + ' mm',
        'Absolute Multiplier': (targetScale / 100).toFixed(2) + 'x'
      };
        ...
        <div className="md:col-span-6 p-6 saas-card space-y-4">
          ...
          <div className="space-y-4">
            <div className="space-y-1.5">
              <label className="text-xs font-bold text-zinc-550 dark:text-zinc-400">Current Dimension (mm)</label>
              <input
                type="number"
                value={currentDim}
                onChange={(e) => setCurrentDim(Number(e.target.value))}
                className="saas-input"
              />
            </div>
            <div className="space-y-1.5">
              <label className="text-xs font-bold text-zinc-550 dark:text-zinc-400">Scale Factor (%)</label>
              <input
                type="number"
                value={targetScale}
