...
export default function NozzleFlowCalculator() {
  // Input States
  const [speed, setSpeed] = useState<number>(150);
  const [layerHeight, setLayerHeight] = useState<number>(0.2);
  const [lineWidth, setLineWidth] = useState<number>(0.42);
  ...
  useEffect(() => {
    try {
      ...
      const flow = speed * layerHeight * lineWidth;
      return {
        'Volumetric Flow Rate': flow.toFixed(2) + ' mm³/s',
        'Filament Extrusion Speed': (flow / 2.405).toFixed(2) + ' mm/s (for 1.75mm)'
      };
        ...
        <div className="md:col-span-6 p-6 saas-card space-y-4">
          ...
          <div className="space-y-4">
            <div className="space-y-1.5">
              <label className="text-xs font-bold text-zinc-550 dark:text-zinc-400">Printing Speed (mm/s)</label>
              <input
                type="number"
                value={speed}
