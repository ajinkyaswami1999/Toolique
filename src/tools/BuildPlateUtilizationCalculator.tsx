...
export default function BuildPlateUtilizationCalculator() {
  // Input States
  const [partX, setPartX] = useState<number>(60);
  const [partY, setPartY] = useState<number>(60);
  const [plateSize, setPlateSize] = useState<number>(256);
  const [margin, setMargin] = useState<number>(6);
  ...
  useEffect(() => {
    try {
      ...
      const bounds = plateSize - (margin * 2);
      const cols = Math.floor(bounds / partX);
      const rows = Math.floor(bounds / partY);
      const fit = cols * rows;
      return {
        'Max Parts Matrix': cols + ' x ' + rows + ' grid',
        'Maximum Units Fit': fit + ' units',
        'Plate Coverage': ((fit * partX * partY) / (plateSize * plateSize) * 100).toFixed(1) + '%'
      };
        ...
        <div className="md:col-span-6 p-6 saas-card space-y-4">
          ...
          <div className="space-y-4">
            <div className="space-y-1.5">
              <label className="text-xs font-bold text-zinc-550 dark:text-zinc-400">Model Width X (mm)</label>
              <input
                type="number"
                value={partX}
                onChange={(e) => setPartX(Number(e.target.value))}
                className="saas-input"
              />
            </div>
            <div className="space-y-1.5">
              <label className="text-xs font-bold text-zinc-550 dark:text-zinc-400">Model Depth Y (mm)</label>
              <input
                type="number"
                value={partY}
            ...
            <div className="space-y-1.5">
              <label className="text-xs font-bold text-zinc-550 dark:text-zinc-400">Build Plate Width (mm)</label>
              <input
                type="number"
                value={plateSize}
            ...
            <div className="space-y-1.5">
              <label className="text-xs font-bold text-zinc-550 dark:text-zinc-400">Safety Edge Margin (mm)</label>
              <input
                type="number"
                value={margin}
