...
export default function FilamentUsageCalculator() {
  // Input States
  const [partWeight, setPartWeight] = useState<number>(85);
  const [quantity, setQuantity] = useState<number>(50);
  const [spoolSize, setSpoolSize] = useState<number>(1000);
  const [spoolCost, setSpoolCost] = useState<number>(1500);
  ...
  useEffect(() => {
    try {
      ...
      const totalWeight = partWeight * quantity;
      const spoolsNeeded = Math.ceil(totalWeight / spoolSize);
      const totalCost = spoolsNeeded * spoolCost;
      return {
        'Total Mass Required': (totalWeight / 1000).toFixed(2) + ' kg',
        'Standard Spools Needed': spoolsNeeded + ' rolls',
        'Total Spools Cost': '₹' + totalCost.toFixed(2),
        'Leftover Filament': ((spoolsNeeded * spoolSize) - totalWeight).toFixed(0) + ' g'
      };
  ...
  }, [partWeight, quantity, spoolSize, spoolCost]);
        ...
        <div className="md:col-span-6 p-6 saas-card space-y-4">
          ...
          <div className="space-y-4">
            <div className="space-y-1.5">
              <label className="text-xs font-bold text-zinc-550 dark:text-zinc-400">Weight Per Part (grams)</label>
              <input
                type="number"
                value={partWeight}
                onChange={(e) => setPartWeight(Number(e.target.value))}
                className="saas-input"
              />
            </div>
            <div className="space-y-1.5">
              <label className="text-xs font-bold text-zinc-550 dark:text-zinc-400">Number of Parts</label>
              <input
                type="number"
                value={quantity}
            ...
            <div className="space-y-1.5">
              <label className="text-xs font-bold text-zinc-550 dark:text-zinc-400">Spool Size (g)</label>
              <input
                type="number"
                value={spoolSize}
                onChange={(e) => setSpoolSize(Number(e.target.value))}
                className="saas-input"
              />
            </div>
            <div className="space-y-1.5">
