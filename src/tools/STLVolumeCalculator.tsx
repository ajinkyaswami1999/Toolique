...
export default function STLVolumeCalculator() {
  ...
  useEffect(() => {
    ...
    if (stlDetails) {
      const densityVal = typeof density === 'number' ? density : 1.24;
      const weight = stlDetails.volume * densityVal;
      setResults({
        'Model Volume': stlDetails.volume.toFixed(2) + ' cm³',
        'Model Net Weight': weight.toFixed(1) + ' grams',
        'Slicer Dimensions': `${stlDetails.x.toFixed(1)} x ${stlDetails.y.toFixed(1)} x ${stlDetails.z.toFixed(1)} mm`
      });
    }
  }, [density, stlFile, stlDetails]);
        ...
        <div className="md:col-span-6 p-6 saas-card space-y-4">
          ...
          <div className="space-y-4">
            <div className="space-y-1.5">
              <label className="text-xs font-bold text-zinc-550 dark:text-zinc-400">Material Density (g/cm³)</label>
              <input
                type="number"
                value={density}
            ...
            <div className="space-y-1.5">
              <label className="text-xs font-bold text-zinc-550 dark:text-zinc-400">Select Binary STL File</label>
              <input
                type="file"
                accept=".stl"
