...
export default function HueForgeFilamentCalculator() {
  // Input States
  const [baseLayer, setBaseLayer] = useState<number>(0.8);
  const [stepLayer, setStepLayer] = useState<number>(0.08);
  const [totalLayers, setTotalLayers] = useState<number>(30);
  ...
  useEffect(() => {
    try {
      ...
      const height = baseLayer + (stepLayer * (totalLayers - 1));
      return {
        'Total Model Height': height.toFixed(2) + ' mm',
        'Step Increments': stepLayer.toFixed(2) + ' mm'
      };
  ...
  }, [baseLayer, stepLayer, totalLayers]);
        ...
        <div className="md:col-span-6 p-6 saas-card space-y-4">
          ...
          <div className="space-y-4">
            <div className="space-y-1.5">
              <label className="text-xs font-bold text-zinc-550 dark:text-zinc-400">Base Layer Height (mm)</label>
              <input
                type="number"
                value={baseLayer}
            ...
            <div className="space-y-1.5">
              <label className="text-xs font-bold text-zinc-550 dark:text-zinc-400">Step Detail Height (mm)</label>
              <input
                type="number"
                value={stepLayer}
            ...
            <div className="space-y-1.5">
              <label className="text-xs font-bold text-zinc-550 dark:text-zinc-400">Total Output Layers Count</label>
              <input
                type="number"
                value={totalLayers}
