...
export default function HueForgeColorSwapPlanner() {
  // Input States
  const [layersList, setLayersList] = useState<string>('10, 16, 22');
  const [baseLayer, setBaseLayer] = useState<number>(0.8);
  const [stepLayer, setStepLayer] = useState<number>(0.08);
  ...
  useEffect(() => {
    try {
      ...
      const layers = layersList.split(',').map(x => parseInt(x.trim())).filter(x => !isNaN(x));
      const instructions = layers.map((lay) => {
        const height = baseLayer + (lay - 1) * stepLayer;
        return 'Layer ' + lay + ' (' + height.toFixed(2) + 'mm)';
      });
      return {
        'Swap Checklist': instructions.join(' | ') || 'No swap layers defined'
      };
  ...
  }, [layersList, baseLayer, stepLayer]);
        ...
        <div className="md:col-span-6 p-6 saas-card space-y-4">
          ...
          <div className="space-y-4">
            <div className="space-y-1.5">
              <label className="text-xs font-bold text-zinc-550 dark:text-zinc-400">Swap Layers (comma separated)</label>
              <input
                type="text"
                value={layersList}
                onChange={(e) => setLayersList(e.target.value)}
                className="saas-input"
              />
            </div>
            <div className="space-y-1.5">
              <label className="text-xs font-bold text-zinc-550 dark:text-zinc-400">Base Layer (mm)</label>
              <input
                type="number"
                value={baseLayer}
                onChange={(e) => setBaseLayer(Number(e.target.value))}
                className="saas-input"
              />
            </div>
            <div className="space-y-1.5">
              <label className="text-xs font-bold text-zinc-550 dark:text-zinc-400">Step Layer (mm)</label>
              <input
                type="number"
                value={stepLayer}
                onChange={(e) => setStepLayer(Number(e.target.value))}
                className="saas-input"
              />
            </div>
          </div>
