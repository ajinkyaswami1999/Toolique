...
export default function FlushVolumeCalculator() {
  // Input States
  const [fromColor, setFromColor] = useState<string>('Black');
  const [toColor, setToColor] = useState<string>('White');
  ...
  useEffect(() => {
    try {
      ...
      let flush = 200; // standard
      if (fromColor === 'Black' && toColor === 'White') {
        flush = 800; // deep purge needed
      } else if (fromColor === 'White' && toColor === 'Black') {
        flush = 120; // minimal purge needed
      } else if (fromColor === toColor) {
        flush = 0;
      }
      return {
        'Recommended Flush Volume': flush + ' mm³',
        'AMS Cleaning Directive': (flush > 400 ? 'High cleaning purge needed to prevent color bleed' : 'Standard clean purge is sufficient')
      };
        ...
        <div className="md:col-span-6 p-6 saas-card space-y-4">
          ...
          <div className="space-y-4">
            <div className="space-y-1.5">
              <label className="text-xs font-bold text-zinc-550 dark:text-zinc-400">From Color</label>
              <select
                value={fromColor}
            ...
            <div className="space-y-1.5">
              <label className="text-xs font-bold text-zinc-550 dark:text-zinc-400">To Color</label>
              <select
                value={toColor}
