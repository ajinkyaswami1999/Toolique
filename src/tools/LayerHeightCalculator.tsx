...
export default function LayerHeightCalculator() {
  // Input States
  const [nozzleSize, setNozzleSize] = useState<number>(0.4);
  ...
  useEffect(() => {
    try {
      ...
      const minHeight = nozzleSize * 0.25;
      const maxHeight = nozzleSize * 0.75;
      const optimal = nozzleSize * 0.5;
      return {
        'Minimum Height (Detail)': minHeight.toFixed(2) + ' mm',
        'Recommended (Balanced)': optimal.toFixed(2) + ' mm',
        'Maximum Height (Draft)': maxHeight.toFixed(2) + ' mm'
      };
        ...
        <div className="md:col-span-6 p-6 saas-card space-y-4">
          ...
          <div className="space-y-4">
            <div className="space-y-1.5">
              <label className="text-xs font-bold text-zinc-550 dark:text-zinc-400">Nozzle Diameter (mm)</label>
              <input
                type="number"
                value={nozzleSize}
