...
export default function MachineUtilizationCalculator() {
  // Input States
  const [activeHours, setActiveHours] = useState<number>(120);
  const [totalPrinters, setTotalPrinters] = useState<number>(5);
  ...
  useEffect(() => {
    try {
      ...
      const maxHours = totalPrinters * 24 * 7;
      const util = (activeHours / maxHours) * 100;
      return {
        'Max Possible Hours/Week': maxHours + ' hours',
        'Uptime Print Hours': activeHours + ' hours',
        'Farm Uptime Utilization': util.toFixed(1) + '%'
      };
        ...
        <div className="md:col-span-6 p-6 saas-card space-y-4">
          ...
          <div className="space-y-4">
            <div className="space-y-1.5">
              <label className="text-xs font-bold text-zinc-550 dark:text-zinc-400">Uptime Print Hours/Week</label>
              <input
                type="number"
                value={activeHours}
            ...
            <div className="space-y-1.5">
              <label className="text-xs font-bold text-zinc-550 dark:text-zinc-400">Total Printers Available</label>
              <input
                type="number"
                value={totalPrinters}
                onChange={(e) => setTotalPrinters(Number(e.target.value))}
                className="saas-input"
              />
            </div>
          </div>
