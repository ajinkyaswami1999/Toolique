...
export default function NozzleSizeComparison() {
  // Input States
  const [currentSize, setCurrentSize] = useState<number>(0.4);
  const [targetSize, setTargetSize] = useState<number>(0.6);
  ...
  useEffect(() => {
    try {
      ...
      const ratio = Math.pow(targetSize / currentSize, 2);
      const timeReduction = (1 - (1 / ratio)) * 100;
      return {
        'Volumetric Flow Multiplier': ratio.toFixed(2) + 'x',
        'Estimated Time Saving': Math.max(0, timeReduction).toFixed(0) + '% faster print speeds',
        'Detail Resolution Change': (targetSize > currentSize ? 'Coarser details, wider layers' : 'Finer details, thinner layers')
      };
        ...
        <div className="md:col-span-6 p-6 saas-card space-y-4">
          ...
          <div className="space-y-4">
            <div className="space-y-1.5">
              <label className="text-xs font-bold text-zinc-550 dark:text-zinc-400">Current Nozzle Size (mm)</label>
              <input
                type="number"
                value={currentSize}
            ...
            <div className="space-y-1.5">
              <label className="text-xs font-bold text-zinc-550 dark:text-zinc-400">Target Nozzle Size (mm)</label>
              <input
                type="number"
                value={targetSize}
