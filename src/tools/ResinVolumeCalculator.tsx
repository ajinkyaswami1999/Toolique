...
export default function ResinVolumeCalculator() {
  // Input States
  const [volume, setVolume] = useState<number>(60);
  const [density, setDensity] = useState<number>(1.12);
  ...
  useEffect(() => {
    try {
      ...
      return {
        'Estimated Resin Weight': weight.toFixed(1) + ' grams'
      };
        ...
        <div className="md:col-span-6 p-6 saas-card space-y-4">
          ...
          <div className="space-y-4">
            <div className="space-y-1.5">
              <label className="text-xs font-bold text-zinc-550 dark:text-zinc-400">Resin Volume (ml)</label>
              <input
                type="number"
                value={volume}
            ...
            <div className="space-y-1.5">
              <label className="text-xs font-bold text-zinc-550 dark:text-zinc-400">Resin Density (g/ml)</label>
              <input
                type="number"
                value={density}
