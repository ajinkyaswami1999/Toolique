...
export default function PackagingCostCalculator() {
  // Input States
  const [boxPrice, setBoxPrice] = useState<number>(25);
  const [bubbleWrap, setBubbleWrap] = useState<number>(10);
  const [labelTape, setLabelTape] = useState<number>(5);
  ...
  useEffect(() => {
    try {
      ...
      const total = boxPrice + bubbleWrap + labelTape;
      return {
        'Total Packaging Material Cost': '₹' + total.toFixed(2)
      };
        ...
        <div className="md:col-span-6 p-6 saas-card space-y-4">
          ...
          <div className="space-y-4">
            <div className="space-y-1.5">
              <label className="text-xs font-bold text-zinc-550 dark:text-zinc-400">Cardboard Box Cost (₹)</label>
              <input
                type="number"
                value={boxPrice}
            ...
            <div className="space-y-1.5">
              <label className="text-xs font-bold text-zinc-550 dark:text-zinc-400">Bubble Wrap & Fillers (₹)</label>
              <input
                type="number"
                value={bubbleWrap}
                onChange={(e) => setBubbleWrap(Number(e.target.value))}
                className="saas-input"
              />
            </div>
            <div className="space-y-1.5">
              <label className="text-xs font-bold text-zinc-550 dark:text-zinc-400">Labels, Tape, & Stickers (₹)</label>
              <input
                type="number"
                value={labelTape}
                onChange={(e) => setLabelTape(Number(e.target.value))}
                className="saas-input"
              />
            </div>
          </div>
