...
export default function ExposureTimeHelper() {
  // Input States
  const [screenType, setScreenType] = useState<string>('Mono LCD');
  const [layerThickness, setLayerThickness] = useState<number>(0.05);
  ...
  useEffect(() => {
    try {
      ...
      let base = 25;
      let normal = 2.5;
      if (screenType === 'RGB LCD') {
        base = 60;
        normal = 8;
      }
      return {
        'Bottom Layers Exposure': base.toFixed(0) + ' seconds',
        'Normal Layer Exposure': normal.toFixed(1) + ' seconds',
        'Bottom Layers Count': '5 - 6 layers'
      };
        ...
        <div className="md:col-span-6 p-6 saas-card space-y-4">
          ...
          <div className="space-y-4">
            <div className="space-y-1.5">
              <label className="text-xs font-bold text-zinc-550 dark:text-zinc-400">LCD Screen Technology</label>
              <select
                value={screenType}
                onChange={(e) => setScreenType(e.target.value)}
                className="saas-select"
              >
                <option value="Mono LCD">Mono LCD</option>
                <option value="RGB LCD">RGB LCD</option>
              </select>
            </div>
            <div className="space-y-1.5">
