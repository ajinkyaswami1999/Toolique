...
export default function MonthlyProductionCalculator() {
  // Input States
  const [printTime, setPrintTime] = useState<number>(4);
  const [printers, setPrinters] = useState<number>(8);
  const [uptimeRate, setUptimeRate] = useState<number>(80);
  ...
  useEffect(() => {
    try {
      ...
      const dailyHoursTotal = printers * 24 * (uptimeRate / 100);
      const dailyUnits = dailyHoursTotal / printTime;
      const monthlyUnits = dailyUnits * 30;
      return {
        'Daily Production Capacity': dailyUnits.toFixed(1) + ' units/day',
        'Estimated Monthly Yield': Math.floor(monthlyUnits) + ' units/month'
      };
        ...
        <div className="md:col-span-6 p-6 saas-card space-y-4">
          ...
          <div className="space-y-4">
            <div className="space-y-1.5">
              <label className="text-xs font-bold text-zinc-550 dark:text-zinc-400">Unit Print Duration (hours)</label>
              <input
                type="number"
                value={printTime}
            ...
            <div className="space-y-1.5">
              <label className="text-xs font-bold text-zinc-550 dark:text-zinc-400">Number of Active Machines</label>
              <input
                type="number"
                value={printers}
            ...
            <div className="space-y-1.5">
              <label className="text-xs font-bold text-zinc-550 dark:text-zinc-400">Expected Daily Uptime (%)</label>
              <input
                type="number"
                value={uptimeRate}
                onChange={(e) => setUptimeRate(Number(e.target.value))}
                className="saas-input"
              />
            </div>
          </div>
