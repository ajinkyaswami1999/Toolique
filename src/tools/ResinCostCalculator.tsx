...
export default function ResinCostCalculator() {
  // Input States
  const [bottleVolume, setBottleVolume] = useState<number>(1000);
  const [bottlePrice, setBottlePrice] = useState<number>(2800);
  const [printVolume, setPrintVolume] = useState<number>(45);
  ...
  useEffect(() => {
    try {
      ...
      const costPerMl = bottlePrice / bottleVolume;
      const totalCost = printVolume * costPerMl;
      return {
        'Cost Per ML': '₹' + costPerMl.toFixed(2),
        'Total Resin Cost': '₹' + totalCost.toFixed(2)
      };
  ...
  }, [bottleVolume, bottlePrice, printVolume]);
        ...
        <div className="md:col-span-6 p-6 saas-card space-y-4">
          ...
          <div className="space-y-4">
            <div className="space-y-1.5">
              <label className="text-xs font-bold text-zinc-550 dark:text-zinc-400">Bottle Size (ml)</label>
              <input
                type="number"
                value={bottleVolume}
            ...
            <div className="space-y-1.5">
              <label className="text-xs font-bold text-zinc-550 dark:text-zinc-400">Bottle Price (₹)</label>
              <input
                type="number"
                value={bottlePrice}
            ...
            <div className="space-y-1.5">
              <label className="text-xs font-bold text-zinc-550 dark:text-zinc-400">Print Volume (ml)</label>
              <input
                type="number"
                value={printVolume}
