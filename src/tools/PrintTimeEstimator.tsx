...
export default function PrintTimeEstimator() {
  // Input States
  const [totalHeight, setTotalHeight] = useState<number>(100);
  const [layerHeight, setLayerHeight] = useState<number>(0.2);
  const [avgSpeed, setAvgSpeed] = useState<number>(60);
  const [complexity, setComplexity] = useState<number>(1.5);
  ...
  useEffect(() => {
    try {
      ...
      const layers = totalHeight / layerHeight;
      const secondsPerLayer = (1000 / avgSpeed) * complexity;
      const totalSeconds = layers * secondsPerLayer;
      const hours = Math.floor(totalSeconds / 3600);
      const minutes = Math.floor((totalSeconds % 3600) / 60);
      return {
        'Total Layers': Math.ceil(layers) + ' layers',
        'Seconds Per Layer': secondsPerLayer.toFixed(1) + ' s',
  ...
  }, [totalHeight, layerHeight, avgSpeed, complexity]);
        ...
        <div className="md:col-span-6 p-6 saas-card space-y-4">
          ...
          <div className="space-y-4">
            <div className="space-y-1.5">
              <label className="text-xs font-bold text-zinc-550 dark:text-zinc-400">Model Height (mm)</label>
              <input
                type="number"
                value={totalHeight}
            ...
            <div className="space-y-1.5">
              <label className="text-xs font-bold text-zinc-550 dark:text-zinc-400">Layer Height (mm)</label>
              <input
                type="number"
                value={layerHeight}
            ...
            <div className="space-y-1.5">
              <label className="text-xs font-bold text-zinc-550 dark:text-zinc-400">Average Speed (mm/s)</label>
              <input
                type="number"
                value={avgSpeed}
            ...
            <div className="space-y-1.5">
              <label className="text-xs font-bold text-zinc-550 dark:text-zinc-400">Detail Complexity Multiplier</label>
              <input
                type="number"
                value={complexity}
