...
export default function LineWidthCalculator() {
  ...
  useEffect(() => {
    try {
      ...
      const optimal = nozzleSize * 1.2;
      const minWidth = nozzleSize * 1.05;
      const maxWidth = nozzleSize * 1.5;
      return {
        'Optimal Line Width': optimal.toFixed(2) + ' mm',
        'Minimum Line Width': minWidth.toFixed(2) + ' mm',
        'Maximum Line Width': maxWidth.toFixed(2) + ' mm'
      };
        ...
        <div className="md:col-span-6 p-6 saas-card space-y-4">
          ...
          <div className="space-y-4">
            <div className="space-y-1.5">
              <label className="text-xs font-bold text-zinc-550 dark:text-zinc-400">Nozzle Size (mm)</label>
              <input
                type="number"
                value={nozzleSize}
